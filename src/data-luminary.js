...
const LUMINARY_DATA = {
  ...
  milestone: {
    ...
    reward: "Permanently add <strong>Drawn into the Light</strong> (Level M card) to your supply. It provides the class's only non-Loss Ranged Attack — Attack 3 Range 2 Pull 1 with Wild consumptions boosting both range and damage — filling a critical gap for scenarios where reaching melee isn't possible. More importantly, the Persistent Loss bottom allows you to have two Glow cards active simultaneously, unlocking a fundamentally different level of Glow cycling. The Wild Element it generates on play means you can immediately activate your second Glow on the very next turn. A significant upgrade for both builds.",
